module.exports = {
    ...
    getItemData: `select CodigoDeArticulo, Descripcion1, Descripcion2, CodigoAlternativo, CodigoDeDepartamento, CodigoDeFamilia, Codigodesubfamilia, IDUnidad, IDUnidadA, equivalenciaA, CodigoDeBarrasA, IDUnidadB, equivalenciaB,codigodebarrasB, ROUND(longitud, 5) AS longitud, ROUND(ancho, 5) AS ancho, ROUND(alto, 5) AS alto, ROUND(pesoneto, 5) AS pesoneto, ROUND(PesoDeEmbalaje, 5) AS PesoDeEmbalaje, ROUND(PesoBruto, 5) AS PesoBruto, UnidadDeDimensiones, UnidadDePeso from APPIA_SQL.dbo.Articulos where Empresa = @Empresa and `,
